...
for i in `ls I-H-1*/intermediates/I*.r3`
do

NAME=`echo ${i} |awk -F'/' '{print $NF}' | awk -F'_50_vs_' '{print $1}'`
GZ=`ls I-H-*_brass50/intermediates/I*.groups.gz|grep ${NAME}` 
...
less ${i} | sed 's/# chr/chr/g' |grep -v "^#" | awk -F'\t' '{if($8>5) print $0}'|awk -F'\t' '{flag=0;if($1==14 && $4==14) {flag=1;} if(flag==0) {print $0;} else {flag=0;}}' > temp 
less temp | awk -F'\t' '{OFS="\t";if($1~/^chr/) {$(NF+1)="svtype";} else if($1!=$4) {$(NF+1)="translocation"} else if($9=="+" && $10=="+") {$(NF+1)="deletion"} else if($9=="-" && $10=="-") {$(NF+1)="tandem-duplication"} else {$(NF+1)="inversion"} print $0}' > ${NAME}.filtered 
