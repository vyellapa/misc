...
PROJ="251"
for z in `ls *.filtered`
do

echo "Sample ${z}"
GR=`echo ${z} |awk -F'.' '{print $1}'` 


#BAM=`lk get data -p ${PROJ} -v |grep ${GR} |awk -F'\t' '{print $2}'`
BAM=`cat bam_paths |grep ${GR} |awk -F'\t' '{print $1}'`
NAME=`echo ${z} |sed 's/filtered/filtered1/g'`
NAME1=`echo ${z} |sed 's/filtered/filtered2/g'`

cat ${z} | grep chr | awk '{print $0"\tMAPQ1\tMATCH1\tSUPP1\tMAPQ2\tMATCH2\tSUPP2"}' > ${NAME}


for i in `cat ${z} | tr '\t' '!'|sed 's/ //g'|awk -F'!' '{if($48=="deletion") {print $0}}'`
do
...
for i in `cat ${z} | tr '\t' '!'|sed 's/ //g'|awk -F'!' '{if($48=="translocation") {print $0}}'`
do
...
cat ${z} | awk -F'\t' '{if($48!="translocation" && $48!="deletion" && $1!~/^chr1/) {print $0"\tNA\tNA\tNA\tNA\tNA\tNA";}}' >> ${NAME}
