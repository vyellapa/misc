...
PROJ="251"
for z in `ls p251_delly_results_oct19_repeatTrack.tsv`
do


cat ${z} | grep CHR | awk '{print $0"\tMAPQ1\tMATCH1\tSUPP1\tMAPQ2\tMATCH2\tSUPP2"}' > alll


for i in `cat ${z} | tr '\t' '!'|sed 's/ //g'|awk -F'!' '{if($7=="DEL") {print $0}}'`
do
